...
import { solveNash } from '../lib/nashSolver'
    ...
    <div className="rounded-lg border border-white/10 px-4 py-3 flex items-center gap-4">
      <div className="flex-1">
        ...
        <p className="text-xs text-muted mt-0.5">
          Einmalige Vorberechnung aller Hand-vs-Hand-Equities (~{Math.ceil((TOTAL_PAIR_COUNT - Math.round(pct * TOTAL_PAIR_COUNT)) * 8 / 1000)} s). Danach instantane Nash-Ranges.
        </p>
        {running && (
          <div className="mt-2">
...
export function SpotAnalyzer(): JSX.Element {
  ...
  // Nash-Ranges für aktuellen Spot vorab laden (ohne Hand-Analyse)
  ...
  function handleAnalyze(): void {
    ...
    setTimeout(() => {
      ...
      if (heroCombos.length > 0) {
        ...
        // Calling-Range aus Nash-Ergebnis aufbauen
        const callRangeHands = [...nashResult.callRange.entries()]
          .filter(([, r]) => r.ev > 0)
          .map(([id]) => id)

        if (callRangeHands.length > 0) {
          // Wir nutzen das bestehende buildCallingRange mit einem synthetischen Spot
          // (Nash-Calling-Range hat ev > 0 für alle call-Hände)
          const syntheticSpot = {
            players, position, stackBb, action: 'call' as const,
          ...
          if (range.length > 0) {
            equityResult = computeEquityMC(heroCards, range, 2000)
          }
        }

        // ICM-Szenarien
        if (fullStacks.every(s => s > 0)) {
          icmResult = computeIcmScenarios(
            fullStacks, payouts, bbSize, ante, callerIdx, computeIcmEquities,
          )
        }
      }
  ...
  return (
    <div className="card p-5 md:p-6 flex flex-col gap-6">
      ...
      <div className="flex flex-wrap gap-4 items-end">
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-2">
            <input type="range" min={2} max={25} step={0.5} value={stackBb} onChange={e => handleStackBbChange(parseFloat(e.target.value))} className="w-28 accent-accent" />
            <input type="number" min={2} max={25} step={0.5} value={stackInput} onChange={e => { setStackInput(e.target.value); const n=parseFloat(e.target.value); if(!isNaN(n)) handleStackBbChange(n) }} className="bg-surface border border-white/10 rounded-lg px-2 py-1.5 text-sm text-text tabnum focus:outline-none focus:ring-1 focus:ring-accent w-16" />
          </div>
        </div>
        <div className="flex flex-col gap-1">
      ...
      <div className="flex flex-wrap gap-4 items-end">
        ...
        <button className="btn-ghost text-xs self-end mb-0.5 disabled:opacity-50" onClick={handleLoadNash} disabled={nashLoading}>
      ...
      <div>
        ...
        <p className="text-xs text-muted mb-3">
          Hintergrundfarbe = Nash-Push-EV für diesen Spot (grün = pushen, gelb = grenzwertig, grau = folden / keine Daten).
        </p>
      ...
      {result && (
        <div className="flex flex-col gap-5 border-t border-white/10 pt-5">
            ...
            {selectedNash && (
              <>
                ...
                <span className="tabnum text-sm text-muted">
                  Nash-EV: <span className={selectedNash.ev >= 0 ? 'text-profit' : 'text-loss'}>
                    {selectedNash.ev >= 0 ? '+' : ''}{selectedNash.ev.toFixed(3)} (Payout-Einheit)
                  </span>
                </span>
                <span className="tabnum text-sm text-muted">
          ...
          {/* Nash-Konvergenz-Info */}
          <p className="text-xs text-muted">
            Nash-Solver konvergiert nach <span className="text-text tabnum">{result.nashResult.iterations}</span> Iterationen
            {result.nashResult.converged ? ' ✓' : ' (max. Iterationen erreicht)'}.
            Push-Range: <span className="text-text tabnum">{[...result.nashResult.pushRange.values()].filter(r => r.ev > 0).length}</span> Hände ·
            Call-Range: <span className="text-text tabnum">{[...result.nashResult.callRange.values()].filter(r => r.ev > 0).length}</span> Hände.
          </p>

          {/* MC-Equity-Balken */}
          {result.equity && (
            <div className="rounded-lg border border-white/10 p-4 flex flex-col gap-2">
              <p className="text-xs font-medium text-muted">Equity vs Nash-Calling-Range (Monte Carlo, {result.equity.iterations.toLocaleString('de-DE')} Iterationen)</p>
              ...
              {result.equity && selectedNash && (
                <div className="rounded-lg bg-white/[0.03] px-3 py-2.5 flex items-center gap-3">
                  ...
                  {(() => {
                    ...
                    // pCall aus Nash-Solver (konsistenter als MC-callFraction)
          ...
          <p className="text-xs text-muted">
            Nash-Ranges berechnet via iterativer Best-Response (ICM-adjustiert, Malmuth-Harville).
            Equity via Monte Carlo gegen die Nash-Calling-Range.
          </p>
        </div>
      )}
    </div>
